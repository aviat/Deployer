...
class VcExportCommand(Command):
	...
	def __init__(self, project, source, destination, _from, to, revision = 0, retry = 1):
		...
		@param revision: revision number to export, 0 to export the last revision.
		...
		super(VcExportCommand, self).__init__(project, source, destination)

	@classmethod
	def get_instance(self, project, parameters):
		"""
		Creates an instance of a VcExportCommand object and returns it.
		...
		@return: An instance of VcExportCommand
	...
	def execute_remote_to_local(self, project):
		"""
		Export a directory on the subversion server to local location.
		...
		export_path = os.path.join(subversion_server.get_repository_path(), self.__from.lstrip("/"))

		AppLogger.info('Checking out "%s" to "%s".' % (export_path, self.__to))

		return subversion_server.export(self.__to, self.__from, self.__revision, self.__retry)
